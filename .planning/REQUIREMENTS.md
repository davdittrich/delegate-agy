...
| S1 | Phase 2 | — | partial — tab/extra-column normalization and distinct degraded-list reporting proven end to end on the bridge (plan 02-01: R9c, R8/R9b); shim mirror pending plan 02-02 |
| S2 | Phase 4 | — | shipped on branch |
| S3 | Phase 5 | `cy5` (shared with R11) | open — contract table and per-mode tests not written |
| S4 | Phase 2 | `8ph` | partial — bridge's write gate (D-03), stale-cache fallback (D-04) and atomic-write preservation shipped (plan 02-01: R9, R9b); `delegate-agy-8ph` forbids a one-sided fix, so `8ph` stays open until plan 02-02 mirrors the gate into `gemini_shim.sh` |
