...
| Model name rejected | `agy-bridge` and the `gemini` shim diverge here because the bridge validates `--model` against its own live list and exits `2` immediately with `ERROR: unknown --model '<NAME>'; run 'agy models' for valid names` — agy is never invoked. The shim does not validate: with a usable model list it warns `WARNING: model '<NAME>' did not resolve against the agy model list; passing it through unchanged` on stderr and lets agy decide, so this row's message is only ever seen through the shim path, where agy itself replies `Error: invalid model selection (--model "NAME" --effort ""): model NAME is not recognized as a known model or custom model in settings`, followed by an `Available models:` list; with no usable list (the row above) the same name passes through silently instead, with no warning at all. Run `agy models` for the current ids and display names. |
| Exit code 2 (`agy model list contains no 'gemini-' ids; agy may be unauthenticated`) | The fetched (or cached) model list has no `gemini-`-prefixed ids — agy is degraded or unauthenticated, not a bad `--type`. Only `agy-bridge` emits this message and exits 2 on it; the `gemini` shim has no matching check and degrades quietly on the same input instead. `agy-bridge` and the `gemini` shim diverge here because a delegation call needs a real model id and the bridge hard-fails rather than guess one, while the shim gates its own unknown-model warning off on a `gemini`-less list — a `gemini-`-less reply is degraded agy, not evidence that a real alias like `flash` is unknown, so the shim lets it reach agy unchanged and silent for as long as the list stays degraded. Run `agy models` directly to see its raw output, and re-authenticate if needed. |
