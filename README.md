...
| Model name rejected | `agy-bridge` and the `gemini` shim diverge here because the bridge validates `--model` against its own live list and exits `2` immediately with `ERROR: unknown --model '<NAME>'; run 'agy models' for valid names` — agy is never invoked. The shim does not validate: it emits `WARNING: model '<NAME>' did not resolve against the agy model list; passing it through unchanged` on stderr and lets agy decide, so this row's message is only ever seen through the shim path, where agy itself replies `Error: invalid model selection (--model "NAME" --effort ""): model NAME is not recognized as a known model or custom model in settings`, followed by an `Available models:` list. Run `agy models` for the current ids and display names. |
