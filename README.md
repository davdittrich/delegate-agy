...
| `ERROR: agy-delegate ... is installed, but this launcher is pinned to ...` | The plugin was updated but the wrapper is still pinned to the old version; it refuses to run the stale copy (exit `127`) until you repin — run the command it prints, or re-run `/agy-setup`'s install command. `agy-bridge` and the `gemini` shim behave identically here because both wrappers are generated by the same `write_wrapper()` implementation in the installer, invoked once per launcher — one code path reached from two call sites, not two behaviours. |
...
| Model name rejected | The `gemini` shim never rejects a name it merely does not recognize — it passes the name through to agy unchanged, which is why the error you see here is agy's own: `Error: invalid model selection (--model "NAME" --effort ""): model NAME is not recognized as a known model or custom model in settings`, followed by an `Available models:` list. It warns on stderr with `WARNING: model '<NAME>' did not resolve against the agy model list; passing it through unchanged` only when a usable model list exists that does not contain the name; with no usable list (the row below) the same name passes through silently instead. `agy-bridge` differs here because it refuses an unknown `--model` itself with exit `2` (`ERROR: unknown --model '<NAME>'; run 'agy models' for valid names`) before agy is ever called: the shim shadows the system `gemini` for every PATH caller that never opted into agy, so it must not refuse a name it does not recognize, while the bridge is an explicit, watched invocation that can validate up front. Run `agy models` for the current ids and display names. |
| Exit code 2 (`agy model list contains no 'gemini-' ids; agy may be unauthenticated`) | The fetched (or cached) model list has no `gemini-`-prefixed ids — agy is degraded or unauthenticated, not a bad `--type`. Only `agy-bridge` emits this message and exits 2 on it; the `gemini` shim has no matching check and degrades quietly on the same input instead. `agy-bridge` and the `gemini` shim diverge here because the shim shadows the system `gemini` for every PATH caller (Octopus, Metaswarm, interactive shells) that never opted into agy, so a stderr warning there would be box-wide noise, while the bridge is an explicit, watched invocation where a loud failure costs nothing — a `gemini-`-less list is degraded agy, not evidence that a real alias like `flash` is unknown, so the shim lets the request reach agy unchanged instead of guessing. Run `agy models` directly to see its raw output, and re-authenticate if needed. |
| Exit code 124 (`ERROR: agy timeout after <N>s` on stderr — byte-identical in `agy-bridge` and the `gemini` shim; `agy-bridge --json` instead emits `"error": "Timeout after <N>s"`, a differently-worded JSON literal with no `ERROR: agy` prefix and a capital `T` — the shim has no JSON arm for this branch at all) | Timeout — simplify the query or pass `--timeout 600`. `agy-bridge` and the `gemini` shim diverge here because the bridge's timeout arm forks on `--json` and wraps the message in its JSON envelope, while the shim's timeout arm sits ahead of any output-format check and always writes plain text to stderr with exit `124` — a caller that asked the shim for `--output-format json` still gets no envelope for a timed-out call. |
...
| Exit code 3 (`ERROR: agy returned empty output [<class>]: <reason>` on stderr — byte-identical in `agy-bridge` and the `gemini` shim, where `<class>` is `quota`, `auth`, or `empty_output` and `<reason>` is agy's captured stderr, or a fixed fallback string when stderr was empty too) | agy exited 0 with no output — usually quota `RESOURCE_EXHAUSTED (429)`. The reason (full agy stderr) is surfaced; wait for quota reset or re-auth. Both `agy-bridge` and the `gemini` shim fail loud here rather than reporting empty success. The two entry points' JSON envelopes differ in shape: `agy-bridge --json` emits `{"success":false,...,"error":<reason>,"error_class":<class>}`; the shim's JSON form emits `{"error":{"message":<reason>,"class":<class>}}` — a JSON consumer must not assume one schema fits both. |
| `WARNING: timeout/gtimeout not found -- bounding agy with the bash watchdog fallback; install coreutils for process-group kill` | Not a failure, and not fatal to either entry point: the call is still bounded, by the bash watchdog. `agy-bridge` and the `gemini` shim behave identically here because both carry byte-identical copies of `run_bounded`, so a host without coreutils changes which kill you get, never whether the call is bounded. `brew install coreutils` (macOS) upgrades the kill from the direct process to its whole process group, so anything agy forked cannot outlive the bound either. |

**Generic (undocumented) nonzero exit codes.** Neither row above covers an arbitrary nonzero exit from agy itself (anything other than 2, 3, 124, or 137) because there is no single message to quote — agy's own stderr is relayed, worded two different ways at two different boundaries. Bridge-versus-shim: `agy-bridge`'s plain-text form prefixes agy's stderr with `ERROR: agy exit <N>: ` (omitting the trailing `: ` when agy wrote nothing to stderr); the `gemini` shim instead relays agy's raw stderr to its own stderr with no prefix at all. Within `agy-bridge` itself, text-versus-JSON: the JSON form (`--json`) carries agy's stderr alone in `"error"`, with none of the `ERROR: agy exit <N>` context the plain-text form adds, and the plain-text arm's stderr is trailing-newline-stripped while the JSON arm's is not.
